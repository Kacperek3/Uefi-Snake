    ...
    for (UINTN line = 0; line < LinesCount; line++) {
        ...
        for (UINTN i = 0; rowStr[i] != L'\0'; i++) {
            ...
            if (c >= 0x2800 && c <= 0x28FF) {
                ...
                UINT32 blockColor = 0x00FFFFFF;

                if (line <= 4) {
                    if (i <= leftTileEnd[line]) {
                        blockColor = 0x00FF3333;
                    } else {
                        blockColor = 0x0033FF33;
                    }
                } else {
                    if (i <= leftTileEnd[line]) {
                        blockColor = 0x003388FF;
                    } else {
                        blockColor = 0x00FFDD33;
                    }
                }

                if (pattern & (1 << 0))
                    DrawRectangle(Platform, currentX, currentY, dotScale, dotScale, blockColor);
                if (pattern & (1 << 1))
                    DrawRectangle(Platform, currentX, currentY + (1 * (dotScale + dotGap)),
                                  dotScale, dotScale, blockColor);
                if (pattern & (1 << 2))
                    DrawRectangle(Platform, currentX, currentY + (2 * (dotScale + dotGap)),
                                  dotScale, dotScale, blockColor);
                if (pattern & (1 << 6))
                ...
                UINTN col1X = currentX + dotScale + dotGap;
                if (pattern & (1 << 3))
                    DrawRectangle(Platform, col1X, currentY, dotScale, dotScale, blockColor);
                if (pattern & (1 << 4))
                    DrawRectangle(Platform, col1X, currentY + (1 * (dotScale + dotGap)), dotScale,
                                  dotScale, blockColor);
                if (pattern & (1 << 5))
